...
1. 開啟之前的操作文本，在 `第二部分` 加入幾個變數，其中  `Bucket_name` 填入 `mybuctket1025`，`My_region` 使用預設的 `us-east-1`，`Canonical_ID` 是用戶帳號的唯一識別碼，當 CLI 操作需要為某個 AWS 帳戶授權時，會使用 `Canonical User ID` 來識別該帳戶，例如授予不同 AWS 帳戶對 S3 資源的訪問權限時會用到。
