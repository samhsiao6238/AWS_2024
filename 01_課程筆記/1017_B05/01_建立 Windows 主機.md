...
_對於實例來說，若無特別指定，`User Data` 只會在首次啟動時自動執行；如需在每次啟動皆會運行 `User Data` 需進行設置；同理，當 `User Data` 只運行一次時，對其進行編輯後重啟並不會生效，所以有修改需求時，需預先將語句寫入 `Userdata`；以下說明如何修改 `User Data`。_
