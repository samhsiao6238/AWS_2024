...
_比較兩者差異，先將當前帶有過濾條件的腳本修改為顯示所有學生的資料，然後再來比較兩者的差異_

<br>

1. 修改 `db_scan_filter.js`，這裡先上代碼，後面再進行說明。
    ...
    var DB = (function(){
        var 
            expose = {
                ...
                getSightings: getSightings,
                // 新增公開方法：獲取過濾後的觀察記錄
                getFilteredSightings: getFilteredSightings
            },hide = {
                //init
                setUpHandlers: setUpHandlers
            };
        ...
        // 非同步函數：獲取所有學生的觀察記錄
        async function getSightings(req, res, next){
            console.log('Fetching all sightings');
            var msg_str = "We are retrieving all bird sightings from DynamoDB. One moment...";
            $("[data-role='get-sightings-from-ddb']").text(msg_str);
            ...
            AWS.config.credentials = new AWS.CognitoIdentityCredentials({
                IdentityPoolId : CONFIG.COGNITO_IDENTITY_POOL_ID_STR,
                Logins : {
                "cognito-idp.us-east-1.amazonaws.com/us-east-1_CD8tA8UaZ": token_str_or_null
                }
            });
            ...
            async function getDdbData(){
                try {
                    const data = await docClient.scan(params).promise()
                    msg_str = "Retrieved all bird sightings.";
                    sightings_list = data['Items'];
                    // 格式化日期
                    for ( var i = 0; i < sightings_list.length; i++ ) {
                        sighting_date = new Date(sightings_list[i].date_int * 1000);
                    ...
                    msg_str = "There was a problem retrieving data from DynamoDB.";
                    return { msg_str: msg_str, sightings_list: [] };
                }
            }
            
            var sightingsDdbData = await getDdbData();
            $("[data-role='get-sightings-from-ddb']").text(sightingsDdbData.msg_str);
            
            return sightingsDdbData.sightings_list;
        }

        // 非同步函數：獲取指定學生的觀察記錄（例如 teststudent）
        async function getFilteredSightings(student_name){
            console.log('Fetching filtered sightings for', student_name);
            var msg_str = `We are retrieving bird sightings for ${student_name}. One moment...`;
            $("[data-role='get-sightings-from-ddb']").text(msg_str);
            ...
            // 從 DynamoDB 獲取過濾後的觀察記錄
            async function getDdbData(){
                try {
                    const data = await docClient.scan(params).promise()
                    msg_str = `Retrieved sightings for ${student_name}.`;
                    sightings_list = data['Items'];
                    // 格式化日期
                    for ( var i = 0; i < sightings_list.length; i++ ) {
                        sighting_date = new Date(sightings_list[i].date_int * 1000);
                    ...
                    msg_str = `There was a problem retrieving data for ${student_name}.`;
                    return { msg_str: msg_str, sightings_list: [] };
                }
            }
...
1. 要顯示所有學生的資料，先刪除 `getSightings` 函數中的 `FilterExpression`，讓它可以從 `BirdSightings` 表中獲取所有學生的觀察記錄。另外，新增一個名為 `getFilteredSightings` 的方法，使用 `FilterExpression` 來過濾並顯示指定學生的觀察記錄。

<br>

2. 先使用 `getSightings()` 來顯示所有學生的資料，然後使用 `getFilteredSightings('teststudent')` 來顯示 `teststudent` 的資料。為達到這個目的，可在 HTML 中設置兩個區塊來顯示。

    ```html
    <div>
        <h3>All Students Data:</h3>
        <pre data-role="all-students-data"></pre>
    </div>

    <div>
        <h3>Test Student Data:</h3>
        <pre data-role="teststudent-data"></pre>
    </div>
    ```

<br>

3. 使用 JavaScript 來填充數據。

    ```javascript
    async function showComparison() {
        const allData = await DB.getSightings();
        const testStudentData = await DB.getFilteredSightings('teststudent');

        $("[data-role='all-students-data']").text(JSON.stringify(allData, null, 2));
        $("[data-role='teststudent-data']").text(JSON.stringify(testStudentData, null, 2));
    }
    ```
