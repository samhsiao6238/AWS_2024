...
4. 使用路徑 `media_input_uri` 中指向的 `test.wav` 檔案作為輸入，建立並啟動一個 AWS Transcribe 的轉錄作業。
...
5. 可查看這個 `response` 的內容，其中 `TranscriptionJobStatus` 在進行中會顯示 `IN_PROGRESS`，完成時會顯示 `COMPLETED` 或 `FAILED`。
...
6. 檢查轉錄工作會傳值中 `transcription job` 的狀態，直到顯示 `COMPLETED` 或 `FAILED` 為止。
...
2. 使用 `get_transcription_job` 方法呼叫的結果檢索輸出文件，其中 `transcription_file` 包含的 URI 是轉錄結果的連結，允許下載或查看轉錄文本。
