...
_STS，Security Token Service_

<br>

1. 因為要在本地進行，所以要進行 AWS 帳號驗證取得權限。
...
1. 透過代碼自動取得 Lab 所指派的所有 `角色的名稱` 和 `ARN`，並根據名稱與其他條件篩選範例所需的角色。
    ...
    # 找出匹配的角色 ARN
    database_access_role_arn = find_role_arn(
        database_role_keyword, roles
    )
    translate_access_role_arn = find_role_arn(
        translate_role_keyword, roles
    )
...
2. 篩選出名稱中包含 `labbucket`的 S3 bucket。
...
2. `test.wav` 範例檔案位於的位置如下，該文件包含音頻短語 `測試。你好，你好，你好。這是一個測試。測試，測試，測試。`
...
3. 轉錄需要幾分鐘才能完成，完成後可前往 `Amazon Transcribe` 控制台查看轉錄內容。

    ![](images/img_04.png)

<br>

4. 檢查 S3 Bucket 是否存在，同時是否具備對於 S3 訪問的權限。
...
5. 使用 `test.wav` 檔案作為輸入，建立並啟動一個 AWS Transcribe 的轉錄作業。
...
6. 檢查 `AWS Transcribe` 轉錄工作 `transcription job` 的狀態，直到工作完成或失敗為止。
...
2. 輸出資料夾的格式是根據帳號和作業 ID 建立的，使用此資訊建立一條路徑。
...
3. Amazon Translate 輸出多個檔案，下載 .txt 檔案。
