...
1. 輸出資料夾的格式是根據帳號和作業 ID 建立的，使用此資訊建立一條路徑。
...
4. `es.test.txt` 就是翻譯結果文件；另外，AWS Translate 在執行批次翻譯工作時，會自動生成一個 `details` 資料夾，其中包含與翻譯工作相關的詳細資料，例如翻譯工作的狀態、錯誤報告、作業的輸入和輸出文件摘要等等。

    ![](images/img_47.png)

<br>

5. Amazon Translate 輸出多個檔案，下載 .txt 檔案。
