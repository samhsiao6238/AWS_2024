...
1. 遍歷所有 S3 bucket 中的資源並顯示樹狀結構；確認內容為空，假如有資料代表前次未正確退出帳號。
...
3. 列出所有 Translation Jobs 並顯示其時間和狀態；包含曾經執行過的工作，這部分即便退出帳號＋關閉 Lab 依舊會存在。
...
7. `es.test.txt` 就是翻譯結果文件；另外，AWS Translate 在執行批次翻譯作業時，會自動生成一個 `details` 資料夾，其中包含與翻譯作業相關的詳細資料，例如翻譯作業的狀態、錯誤報告、作業的輸入和輸出文件摘要等等。

    ![](images/img_47.png)

<br>

8. Amazon Translate 輸出多個檔案，下載 .txt 檔案。
...
2. 直接查看 S3 中文件的內容。
    ...
    ```python
    # 定義 bucket 名稱
    bucket_name = bucket  # 假設 bucket 名稱已經在前面定義
    ...
    for my_bucket_object in my_bucket.objects.filter(Prefix=translate_output_path):
        ...
        if file_key.endswith("txt"):
            ...
            try:
                ...
                print(f"文件 '{file_key}' 不存在於 bucket '{bucket_name}' 中。")
            except s3_client.exceptions.ClientError as e:
                error_code = e.response['Error']['Code']
                if error_code == 'AccessDenied':
                    print("讀取文件失敗，權限不足")
...
4. 檢查原本的編碼，由結果得知文件內容本身並沒有編碼問題，因為 `ASCII` 編碼是 `UTF-8` 的子集，ASCII 是一種無特殊字符的基本編碼，並且與 UTF-8 完全兼容。
...
5. 為求謹慎，重新編碼後再上傳一次；似乎依舊沒有排除，將另做處理。
    ...
    try:
        ...
        print(f"成功將 UTF-8 編碼文件 {local_file} 上傳並覆蓋到 S3 路徑 {s3_file_key} 中")
    except s3_client.exceptions.ClientError as e:
        error_code = e.response['Error']['Code']
        if error_code == 'AccessDenied':
            print("上傳文件失敗，無法覆蓋 S3 中的文件，權限不足")
...
6. 最後烏龍收場，這裡並沒有出現亂碼，因為 `¿` 和 `¡` 是西班牙語的句子中正常的結構和符號，用於表示疑問句或驚嘆句的開頭，若顯示的內容如上所示，就代表沒有亂碼的問題。
