...
1. 與官方代碼不同，這裡將修改適用於個人帳號，並且生成 `mybucket` 前綴的隨機 `bucket` 名稱，同時獲取根帳號的 `ARN`。
    ...
    # 獲取當前帳號的 Root ARN 作為 data_access_role_arn
    def get_root_account_arn():
        try:
            response = sts_client.get_caller_identity()
            account_id = response["Account"]
            return f"arn:aws:iam::{account_id}:root"
        except Exception as e:
            print(f"無法取得 Root Account ARN。錯誤訊息：{e}")
            return None

    # 自動取得 bucket 名稱和 data_access_role_arn
    bucket = create_unique_bucket_name()
    create_bucket(bucket)
    data_access_role_arn = get_root_account_arn()

    # 輸出確認
    if bucket:
        print(f"建立的 Bucket 名稱: {bucket}")
    else:
        print("無法建立 Bucket。")

    if data_access_role_arn:
        print(f"取得的 Root Account ARN 作為 Data Access Role ARN: {data_access_role_arn}")
    else:
        print("無法取得 Root Account ARN。")
    ```
...
_這裡是將下載的資料及進行解壓縮，然後取出所需的文件，特別注意，這個文件依舊是個壓縮檔。_
