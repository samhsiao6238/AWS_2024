...
1. 快速理解大規模文本數據的內容概況：透過主題建模生成的主要主題詞彙，可以了解數據集中最常見的討論話題。這樣即使面對龐大的文本數據，仍可透過這些主題迅速掌握數據的概況。舉例來說，在新聞或評論分析中，主題建模可幫助快速理解熱門討論點，例如「科技」、「政治」、「經濟」等主題詞彙。
...
1. `topic-terms.csv`：列出每個主題的前10個關鍵詞，可用於標籤化各主題。
