...
3. 因應權限問題，修改前一個步驟自動化取得的 Bucket 名稱，將其中 `labbucket` 字串改為 `custombucket`，因為在後續步驟中將不再使用，所以直接進行修改；因為原本的 Bucket 中存放了所需的 `.gz` 文件，所以保留原本的變數名稱，將自訂名稱命名為 `bucket_custom`。

    ```python
    # 將 bucket 名稱中的 "labbucket" 替換為 "custombucket"
    bucket_custom = bucket.replace("labbucket", "custombucket")
    bucket_custom
    ```
...
2. 依據官方建議的操作，在解壓縮數據集之前，除了定義存放數據的文件夾外，並清理其中可能存在的舊數據。
...
3. 在 CELL 中可使用 `魔法指令` 下達終端機指令進行解壓縮數據集文件並列出文件內容；我個人是不喜歡這樣的操作，終端機指令還是回到終端機中運行即可。
...
7. 接著將所有新聞組文檔保存到文件 `comprehend_input.txt` 中，每行包含一個文檔。
...
2. 特別注意，在本地運行上傳 S3 的時候出現權限問題，但是從 Sagemaker 是可以儲存的，這裡先透過上傳 `comprehend_input.txt` 到 Sagemaker 中，然後透過代碼轉存到 S3 來排除，之後再回來處理。
