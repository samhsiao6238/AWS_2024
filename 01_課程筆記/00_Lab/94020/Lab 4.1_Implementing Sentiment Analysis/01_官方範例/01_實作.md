...
3. 查看前一個步驟所拆分資料集，包含三個子集 `train`、`validate`、`test`。
...
2. 自訂上傳到 S3 的函數；因為要訓練模型，必須以正確的格式將資料上載到 S3，XGBoost 使用逗號分隔值 (CSV) 檔案。
...
4. 在前面步驟已透過自動化步驟取得 Bucket 名稱，假如不慎運行前一個步驟，可再次取得。
...
6. 將數據上傳到 S3；假如是在 Sagemaker 中要將數據寫入 S3，可使用以下代碼。
...
_以下嘗試使用 Presigned URL 在本地上傳檔案至 S3，這是在無權更改 IAM 設置的情況下，在本地的 Jupyter Notebook 中使用 SageMaker Notebook 的權限上傳檔案至 S3，也就是使用 SageMaker Notebook 生成的 `Presigned URL` 來實現上傳檔案的功能。_

1. 在 SageMaker Notebook 中生成預簽名 URL。
...
2. 使用本地程式碼通過預簽名 URL 上傳：
   - 將生成的預簽名 URL 複製到本地環境。
   - 使用 `requests` 庫將文件上傳至預簽名 URL。
   ...
   with open(local_file_path, 'rb') as file_data:
       response = requests.put("PASTE_PRESIGNED_URL_HERE", data=file_data)
...
- 方法 1 是在 SageMaker Notebook 中直接上傳文件至 S3，適合無法在本地直接操作 S3 的情況。
- 方法 2 使用預簽名 URL，使您在本地也可以通過 SageMaker Notebook 環境生成的預簽名 URL 進行上傳，並且不需要額外的 IAM 權限。

這兩種方式都可以實現文件的上傳而不需要進行 IAM 權限變更。


## 訓練 XGBoost 模型

1. 創建一個 `Estimator` 並使用參數。
