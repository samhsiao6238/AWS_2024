...
3. 使用 `預簽名 URL` 上傳數據，約略需要一分鐘，其中 `batch_in_file` 尚未用到；特別注意，以下代碼進行型別轉換以確保標籤為整數類型，另外將 label 列移到第一欄，確保模型訓練時識別到正確的標籤，完成後上傳到 S3。
...
4. 檢查 label 列在每個 DataFrame 中的位置，確認已修正。
...
5. 建立 XGBoost 估計器；特別注意，這裡先將輸出設置為空置串。
