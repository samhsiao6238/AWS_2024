...
1. 特別注意，Lab 所賦予的角色並不具備寫入數據到 S3 的權限，必須透過 Sagemaker 的 `get_execution_role` 函數取得 ARN 寫入參數、並在 Sagemaker 中運行才能獲取權限，而這個 `get_execution_role` 函數若在本地運行，也無法取得 Sagemaker 的 ARN，而是取得 Lab 角色的 ARN。

2. 因應這個狀況，接下來將使用 `Presigned URL` 處理文件上傳，這必須透過在 `Sagemaker` 中使用具備權限的 `ARN` 所建立的 `Presigned URL` 來完成；這可在無權更改 IAM 設置、無 PutObject 權限的情況下，在本地的 Jupyter Notebook 中上傳檔案至 S3 的方法。

以下是對於您描述的檢查、修改及補充，目的是提升專業性及清晰度：
...
_回到雲端；確認以上步驟可完成文件的寫入，以下正式建立三個預簽名 URL 來寫入數據集_
