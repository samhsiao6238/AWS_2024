...
2. 驗證帳號，務必確保輸出的帳號與 Lab 取得相同，藉此確認 `.env` 設置正確。
...
3. 在本地運行以上代碼會顯示警告，特別注意，雖然依舊返回角色的 ARN，但這並非 Lab 賦予權限的 Role，但這個角色是運行腳本所必需的資訊，之後會進行詳細說明。
...
3. 為了比對官方數據與自行下載數據是否存在差異，先手動下載範例資料集到本地電腦中，接著將文件更名為 `imdb_0.csv` 後拖曳進入專案資料夾中。
...
_特別注意，在 Lab 並不具備寫入數據到 S3 的權限；這裡使用 `Presigned URL` 處理文件上傳；這必須透過在 Sagemaker 中使用具備權限的 arn 所建立的 `Presigned URL` 來完成；這是在無權更改 IAM 設置、無 PutObject 權限的情況下，在本地的 Jupyter Notebook 中上傳檔案至 S3 的方法；或許還有其他方式，之後會再想想辦法。_
...
_**回到雲端**；確認以上步驟可完成文件的寫入，以下正式建立三個預簽名 URL 來寫入數據集_

<br>

1. 建立三個預簽名 URL 並寫入到 Bucket 文件中。 
    ...
    # 創建預簽名 URL
    urls = {
        'train_file': create_presigned_url(bucket_name, prefix, train_file),
        'validate_file': create_presigned_url(bucket_name, prefix, validate_file),
        'test_file': create_presigned_url(bucket_name, prefix, test_file)
    }
...
1. 下載 pre_url.txt 並讀取其中的預簽名 URL；可略作查看，這是一個 JSON 格式文件。
...
3. 上傳數據；約略需要一分鐘。
    ...
    def save_csv_to_url(dataframe, url):
        ...
        if response.status_code == 200:
            ...
            print(f"上傳失敗，狀態碼：{response.status_code}, 錯誤內容：{response.text}")
...
_進入雲端環境；因為要調用 `get_execution_role()`，所以必須在 Sagemaker 中運行。_
...
3. 其中第一個是自定義的 IAM 政策，根據命名略知是為特定 SageMaker 執行角色創建，使用以下代碼查看具體的權限；由結果可知這個角色擁有對 `Comprehend` 和 `Textract` 服務的全部操作權限，以及對特定 `S3 Bucket` 的讀取和寫入權限。

    ```python
    policy_arn = 'arn:aws:iam::211125745218:policy/c133864a3391494l8261467t1w211125745218-SageMakerExecutionPolicy-IGTFcq0lquw0'
...
4. 列出詳細權限。
    ...
    ```json
    {
        "Version": "2012-10-17", 
        "Statement": [
            {"Action": ["comprehend:*"], "Resource": "*", "Effect": "Allow", "Sid": "ComprehendPermissions"}, 
            {"Action": ["s3:Get*", "s3:List*"], "Resource": "*", "Effect": "Allow", "Sid": "S3AllReadPermissions"}, {"Action": ["iam:GetRole"], "Resource": "*", "Effect": "Allow", "Sid": "IAMPermissions"}, 
            {"Action": ["iam:PassRole"], "Resource": "arn:aws:iam::211125745218:role/service-role/c133864a3391494l8261467t1w-ComprehendDataAccessRole-sfdjpVCY6b8I", "Effect": "Allow", "Sid": "ComprehendAssumeRolePermissions"}, 
            {"Action": ["textract:*"], "Resource": "*", "Effect": "Allow", "Sid": "TextractPermissions"}, 
            {"Action": ["comprehend:*", "s3:ListAllMyBuckets"], "Resource": "*", "Effect": "Allow", "Sid": "S3Permissions"}, 
            {"Action": ["s3:PutObject", "s3:GetObjectAcl", "s3:GetObject", "s3:ListBucketMultipartUploads", "s3:AbortMultipartUpload", "s3:ListBucketVersions", "s3:GetObjectTagging", "s3:ListBucket", "s3:GetObjectVersion", "s3:ListMultipartUploadParts"], "Resource": ["arn:aws:s3:::c133864a3391494l8261467t1w211125745218-labbucket-x8ytc3vanom0", "arn:aws:s3:::*/*"], "Effect": "Allow", "Sid": "S3Permissions2"}
        ]
    }
    ```
...
1. 建立一個 `Estimator` 並使用參數；在本地運行時需要使用 ARN，這在前一個步驟已經取得；特別注意，這個預簽名 URL 尚未建立，之後可能出錯。
...
2. 設定 `data_channels`，其中有兩個數據通道，一個訓練資料、一個驗證資料。

```python
train_channel = sagemaker.inputs.TrainingInput(
    # f's3://{bucket}/{prefix}/train/{train_file}',
    f's3://{bucket}/{prefix}/{train_file}',
    content_type='text/csv'
)

validate_channel = sagemaker.inputs.TrainingInput(
    # f's3://{bucket}/{prefix}/validate/{validate_file}',
    f's3://{bucket}/{prefix}/{validate_file}',
    content_type='text/csv'
)
...
5. 設定 XGBoost 的參數，包括學習率、樹的深度等。這些參數對於模型的性能影響很大。
```python
params = {
    # 或者 'multi:softmax' 根據任務
    'objective': 'binary:logistic',
    # 可以選擇其他評估指標
    'eval_metric': 'auc',
    'max_depth': 3,
...
```python
# 將訓練數據轉換為 DMatrix
dtrain = xgb.DMatrix(train_matrix, label=train['label'].values)

# 將驗證數據轉換為 DMatrix
dval = xgb.DMatrix(validate_matrix, label=validate['label'].values)
```

7. 定義 SageMaker XGBoost 估計器。

```python
xgb_model = Estimator(
    image_uri=retrieve('xgboost', boto3.Session().region_name, '1.0-1'),
    role=role_arn,
    instance_count=1,
    instance_type='ml.m5.2xlarge',
...
8. 將 XGBoost 參數設置到模型中。

```python
xgb_model.set_hyperparameters(**params)
```
