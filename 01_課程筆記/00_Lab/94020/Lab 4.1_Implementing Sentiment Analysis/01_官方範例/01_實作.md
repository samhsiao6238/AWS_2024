...
1. 在雲端的 SageMaker Notebook 中生成預簽名 URL；特別注意，這是在雲端運行。
    ...
    # 設定 S3 Bucket 名稱和文件名稱
    bucket_name = "c133864a3391494l8261467t1w211125745218-labbucket-jm08np1djfzj"  # 替換為您的 S3 bucket 名稱
    s3_file_path = "test.txt"   # 文件名稱，可以自行指定

    # 生成預簽名 URL，有效期設定為 1 小時
    presigned_url = s3_client.generate_presigned_url(
        'put_object',
        Params={'Bucket': bucket_name, 'Key': s3_file_path},
        ExpiresIn=3600  # 設置 URL 有效期（秒）
    )

    print("預簽名 URL:", presigned_url)
    ```

2. 通過 `Presigned URL`，使用 `requests` 庫將文件上傳至 `Presigned URL`。
