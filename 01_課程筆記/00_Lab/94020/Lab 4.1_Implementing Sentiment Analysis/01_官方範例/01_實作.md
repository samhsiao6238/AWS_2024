...
1. 為了觀察官方數據與自行下載數據的差異，這裡將範例資料集文件上傳並命名為 `imdb_0.csv`。

![](images/img_24.png)

<br>

2. 透過代碼觀察，結果顯示數據的內容與欄位不一樣，但格式是相同的。
...
4. 由前述步驟可知，官方數據集的欄位名稱是 `text` 和 `label`；以下將 `imdb.csv` 的欄位名稱修改與官方資料集一致。
...
1. 在雲端建立預簽名 URL，第一步先取得 bucket 名稱；特別注意，這是在雲端運行。
...
2. 自動將前一個步驟取得的 bucket 名稱生成預簽名 URL。
...
4. 上傳 pre_url.txt 至 S3，這是為了要讓本地進行讀取或下載。
