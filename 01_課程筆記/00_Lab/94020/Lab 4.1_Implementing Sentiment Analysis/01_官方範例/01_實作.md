...
1. 在 **_雲端_** 建立預簽名 URL，第一步先取得 bucket 名稱；特別注意，這是在雲端運行。
...
2. 在當前工作路徑中建立一個名為 test.txt 的文件，並將字符串 "測試預簽名 URL" 寫入其中。

```python
# 在當前路徑中建立一個文件 test.txt
...
3. 自動將前一個步驟取得的 bucket 名稱生成預簽名 URL，另外 `key` 設置為文件名稱，代表存放在 Bucket 的根目錄中。
