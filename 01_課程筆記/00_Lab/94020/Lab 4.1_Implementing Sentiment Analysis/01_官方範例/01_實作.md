...
_SageMaker SDK 並不直接支持在 TrainingJobAnalytics 中傳入 role 參數來繞過本地權限檢查，可嘗試使用 Lambda 或 API Gateway，將 TrainingJobAnalytics 查詢動作移到 Lambda 函數中，並通過 API 請求獲取結果。這樣可以利用 Lambda 函數的 IAM 角色來查詢訓練任務的指標數據。_
...
_使用 __test__ 資料集計算更多指標_

1. 假如在雲端，可透過雲端自訂函數將數據傳送至 S3。

```python
upload_s3_csv('batch-in.csv', 'batch-in', test_matrix, test, True)
```

2. 在本地，使用以下代碼將數據集上傳到 S3。
...
2. 若要查詢特定狀態如 `InProgress` 的轉換 Job，可以在 `list_transform_jobs` 方法中添加 `StatusEquals` 參數。

```python
response = sagemaker_client.list_transform_jobs(StatusEquals='InProgress')
```

![](images/img_67.png)

3. 若出現 `ResourceLimitExceeded` 代表超過限額，可略作等待。

![](images/img_65.png)

4. 或是調整 `instance_type` 參數使用較小規格的實例。
...
5. 出現失敗時，使用 describe_transform_job 方法來獲取該作業的詳細資訊。
...
失敗原因: ClientError: Failed to download model data(bucket: c133864a3391494l8261467t1w637423426529-labbucket-u3zb5imjbae8, key: lab41/output/xgb-pass2-11-05-2024-12-48-25/output/model.tar.gz). NoSuchBucket (HTTP 
StatusCode: 404)
...
1. 建立 XGBoost 的 `Transformer Job` 執行批次轉換；因為權限問題對於官方範例略作修改，避免調用日誌 API 進行檢查作業，這樣不會嘗試訪問 `logs:DescribeLogStreams` 權限，從而避免 `AccessDeniedException` 錯誤。
...
# 使用唯一名稱，例如在名稱中加上時間戳
unique_job_name = f"xgboost-pass1-{datetime.now().strftime('%Y%m%d%H%M%S')}"
