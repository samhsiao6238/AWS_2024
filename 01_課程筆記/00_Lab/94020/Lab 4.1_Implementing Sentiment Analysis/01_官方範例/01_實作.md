...
2. 為了觀察官方數據與自行下載數據的差異，這裡手動下載範例資料集，並將文件羹名為 `imdb_0.csv` 後拖曳進入專案資料夾中。

    ![](images/img_24.png)

<br>

3. 透過代碼觀察，結果顯示數據的內容與欄位不一樣，但格式是相同的，並且資料筆數 `50,000` 也相同。
...
4. 列出兩個數據的欄位名稱，結果顯示官方數據集的欄位名稱是 `text` 和 `label`，而下載數據集的欄位是 `review`、`sentiment`。
...
5. 將下載數據集 `imdb.csv` 的欄位名稱修改與官方資料集一致。
...
6. 比對數據，提取自行下載的數據集前五筆進行查詢，比對該筆數據是否存在於官方資料集中，並明確指出是資料集的哪一筆；此步驟用已確認是否僅僅是排序問題。
