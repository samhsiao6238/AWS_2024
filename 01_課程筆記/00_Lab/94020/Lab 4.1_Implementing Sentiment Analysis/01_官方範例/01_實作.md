...
3. 在當前工作路徑中建立一個名為 test.txt 的文件，並將字符串 "測試預簽名 URL" 寫入其中。
...
4. 自動將前一個步驟取得的 bucket 名稱生成預簽名 URL，另外 `key` 設置為文件名稱，代表存放在 Bucket 的根目錄中。
...
6. 上傳 pre_url.txt 至 S3，這是為了要讓本地進行讀取或下載。
