...
1. 執行轉換任務，可透過參數設置 `wait=False` 讓工作在背景執行；這個過程使用 `xgb_transformer` 進行 `批次轉換（Batch Transformation`，將儲存在 S3 的 `CSV 格式數據` 輸入到 `預訓練模型` 中進行 `推理（inference）`。
