...
3. 特別注意，這次無需勾選 `Wait for task to complete`，因為這個任務不需要內部輪詢結果；點擊上方頁籤切換到 `{ } Code`。
...
10. `Operator` 選擇 `is present`；然後點擊右下角 `Save conditions`。
...
3. 與前一個步驟相同，在 `ChoiceStateFirstRun` 狀態的右側是一個標記為 `Default` 的方塊，在沿著箭頭下的方塊中拖曳並加入另一個 `Pass` 狀態，然後將 `State name` 設為 `REPLACE ME FALSE STATE`。
