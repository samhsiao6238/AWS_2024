...
_特別注意，做到這裡為止，完成 Task4 沒問題，但到了 Task5 可能出現錯誤，這裡嘗試先做出微調，之後再來檢討究竟官方的教程有沒有錯誤_

<br>

1. 在 Glue 的數據庫表檢查邏輯中，官方教程建議檢查 `$.ResultSet.Rows[0].Data[0].VarCharValue`，這種方法假設查詢結果已經包含了表名；但是這樣的邏輯存在一個問題，如果查詢結果集中沒有任何表返回，也就是 `Rows` 為空，就會直接檢查 `.VarCharValue`，但因為在空的結果集中並不存在 `Data[0]`，更無法檢查 `.VarCharValue`。

<br>

2. 為了避免查詢結果為空時進行無效的檢查，應將條件邏輯修改為先檢查 `$.ResultSet.Rows[0].Data[0]` 是否存在，也就是檢查 `Rows` 是否返回數據，這樣做可先確認查詢結果集中是否有數據，如果結果集中無數據，則直接進入創建表的邏輯，而不會嘗試檢查無效的 `VarCharValue` 字段。
...
3. 將原本的條件從修改為。
    ...
    ```bash
    $.ResultSet.Rows[0].Data[0] is present
    ```

<br>

4. 修改如下，因為檢查的是 `Data[0]` 是否存在，這是一個布林判斷，並不需要指定特定的值，因此 `Value` 欄位不需要選擇任何選項。

    ![](images/img_82.png)

<br>

5. 然後點擊右下角 `Save conditions`。
