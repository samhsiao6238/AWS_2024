...
1. 先進入 `目標 EC2` 中的 `安全群組` 設定 `白名單`；這步驟與前面相同不再贅述，惟先前做查詢時，跳板中已預設開啟 `0.0.0.0`。

    ![](images/img_39.png)

<br>

2. 在 EC2 主控台中進入目標 EC2，點擊連線 `Connect` 以確認可連入目標 EC2。

    ![](images/img_40.png)

<br>

3. 特別注意，預設是 `root`，這裡要修改使用者名稱為 `ec2-user` 連線；不要使用預設的 `root`。
...
1. 先在 `跳板 EC2` 的終端機中運型以下指令以建立密鑰對，所有提示後都按下 `ENTER` 即可；這就是在後面步驟要複製到目標實例的公鑰，如此便可免除輸入密碼並通過驗證。
...
3. 複製公鑰內容；特別注意，假如有多組公鑰，可任意複製一組到遠端作為驗證，但務必確保本地存在私鑰。

    ![](images/img_42.png)

<br>

4. 在 Cloud9 環境中進入 `目標 EC2` 的終端機，使用以下指令編輯授權文件 `authorized_keys`。
...
5. 貼上 `堡壘 EC2` 在前面步驟建立的公鑰；特別注意，文件中的每一列就是一組公鑰，每一列末端無需斷行符號；編輯完成後，使用 NANO 的組合鍵進行 `儲存（control+O）`、`退出（control+X）`。

    ![](images/img_43.png)

<br>

6. 在 EC2 主控台連線目標 EC2 的畫面是終端機，而終端機下方會顯示目標 EC2 的公共與私有 IP，複製其中的 `PublicIPs`。

    ![](images/img_44.png)

<br>

7. 回到 `堡壘 EC2` 的終端機中，輸入以下指令進行連線；因為已經複製了公鑰，無需使用參數指定密鑰文件。
