...
1. 搜尋自 `2001 年` 以來 `Fiji` 在所有高海域的捕撈價值，並將以美元計算的捕撈價值欄位命名為 `ValueAllHighSeasCatch`；在 `area_name` 部分使用 `IS NULL` 表示搜尋不屬於任何國家的高海域資料，`fishing_entity = 'Fiji'` 過濾出來自 `Fiji` 的捕撈數據，`year > 2000` 過濾自 `2001 年` 以來的數據，`CAST` 將捕撈價值轉換為讀者友好的格式，以兩位小數顯示。
