...
2. 在 AWS 中，`假設（Assume）角色` 指的是一個實體 _暫時獲得_ 該角色的權限，這個實體並不持有該角色的權限，而是透過 _**假設**_ 這個角色來臨時擁有其權限；這種機制是透過 AWS 的 `STS（Security Token Service）` 完成的，它會向實體提供一個臨時的安全憑證，使其能夠以該角色的身份執行操作。因此，在信任關係中，`AWS Glue` 等服務是 _**假設**_ 這個角色來獲得必要的權限進行操作，也就是說 Glue 服務並不永久擁有該角色的權限，而是根據需求在指定的操作期間臨時使用該角色的權限。
...
1. 第一個命名為 `data-source-99991`；`99991` 可以是隨機數字，其餘使用預設，然後點擊右下角 `Create bucket`；這裡依據教程的指引使用 _五碼_ 自訂數字，實務上只要名稱可唯一識別即可。

    ![](images/img_08.png)

<br>

2. 比照前一步驟模式，第二個使用相同尾綴 `99991`，完整命名為 `query-results-99991`；其餘不贅述。
...
1. 進入 Cloud9 後，對預設的環境對象點擊 `Open` 開啟 IDE，並在終端中運行以下命令，這會下載所需的三個 CSV 檔案；這裡加上 `&&` 只是懶得再逐一按 `ENTER`，不是必須的。
