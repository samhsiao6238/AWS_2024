...
1. 不同於之前自動進行的 `Build`，這裡要手動點擊 `Build` 構建機器人，看到完成彈窗後，進行以下的測試。

    ![](images/img_24.png)

<br>

2. 對話中輸入 `Make an appointment`，然後選擇 `root canal`。
...
5. 設置機器人版本的 `別名`；這可為特定版本的機器人建立一個易於識別的名稱，這樣利於管理和部署機器人；再次點擊 `Publish`。
...
6. 滑動到下方點擊 `Create identity pool`。
