...
_Amazon Lex Guided Lab: Natural Language Processing；以下是本 Lab 的官方說明步驟，可完成 Amazon Lex 聊天機器人搭建，與使用者進行互動以預約牙科診療；相關說明可參考官方文件 [Getting Started with Amazon Lex](https://docs.aws.amazon.com/lex/latest/dg/getting-started.html)。_
...
## 設置 Amazon Cognito Identity Pool
...
1. 編輯範例網頁檔案，找到 `144` 行的 `IdentityPoolId`，將前面步驟複製備用的 `ARN` 貼上替換；切記，每次更新 Pool 後會有新的 ARN。

    ![](images/img_53.png)

<br>

2. 在 `185、186` 行 `botAlias` 及 `botName`，分別替換為自己的機器人別名與名稱，若依據講義操作則是 `MyBot` 及 `ScheduleAppointment`；這可在 Lex 清單中查詢；完成後儲存文件並重新上傳。
...
5. 確認 `Block` 已經取消勾選，假如還沒則進行取消，然後點擊 `Save changes`。
...
7. 貼上以下代碼，其中 `Resource` 內的 `arn` 要更換為自己的 S3 的名稱，也就是要替換下方的 `lexlab6-241002`；然後點擊 `Save changes`。
