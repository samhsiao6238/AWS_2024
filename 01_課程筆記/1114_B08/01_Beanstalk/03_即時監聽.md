...
1. 建立專案結構。
    ...
    touch .env .gitignore application.py requirements.txt
...
2. 貼在 `.env` 文件中，然後進行剪貼；完成後將下方貼上的內容刪除。
...
5. 進入即時資料庫查看同步情況；每次點擊都會同步更新，手動修正節點，網頁也會同步。
...
3. 添加一個環境變數 `AWS_EXECUTION_ENV`，任意賦值即可，這裡填入的是 `True`。

    ![](images/img_41.png)

<br>

4. 接著複製 `.env` 文件中的 Key 及 Value。

    ![](images/img_42.png)

<br>

5. 另外保留系統自訂添加的 `PYTHONPATH`，然後點擊右下方 `Apply`。
