...
2. 初始化資源，依據以下的指引，在代碼 `handler = WebhookHandler...` 之後添加初始化邏輯。
...
6. 修改 `handle_message()` 函數；可直接覆蓋原本的內容。
...
7. 修改 `requirements.txt`，添加 `boto3`。
...
5. 使用 AWS CLI；特別注意，以下指令加入 `--no-cli-pager` 參數以一次性完整輸出結果。

    ```bash
    aws dynamodb scan --table-name LinebotMessages --region us-east-1 --output json --no-cli-pager
    ```
