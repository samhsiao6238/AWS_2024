...
2. 接著回到筆記本中，將以上代碼複製到變數 `lambda_code` 的區塊中，並生成壓縮檔 `new_lambda_function.zip`，這將作為指定的 `Lambda Function` 邏輯。
    ...
    if not os.path.exists(source_file):
        raise FileNotFoundError(
            f"{source_file} 不存在，請確認檔案存在當前目錄中。"
        )

    # 壓縮來源文件為 ZIP 文件
    with zipfile.ZipFile(zip_file_name, "w") as zipf:
        # 壓縮時更名為 lambda_function.py
        zipf.write(source_file, arcname="lambda_function.py")
...
2. 檢查 Lambda 函數是否已具備 S3 調用權限。
    ...
    try:
        ...
        print(f"Lambda 函數 {function_name} 當前的資源型策略：")
        print(policy)
        ...
        permission_exists = any(
            ...
            stmt.get('Principal') == 's3.amazonaws.com' and
        ...
        print(
            f"錯誤：Lambda 函數 {function_name} 不存在。"
            "請確認函數名稱是否正確。"
        )
    except Exception as e:
        print(f"發生錯誤：{e}")
...
1. 完成以上測試後，進行環境清理，恢復原狀。
    ...
    if __name__ == "__main__":
        ...
        # 刪除 Lambda 授權
        remove_lambda_permission(function_name, 'S3InvokePermission')
...
2. 若是要刪除指定的 Function。
    ...
    def delete_lambda_function(function_name):
        try:
            ...
            print(
                f"刪除 Lambda function "
                f"'{function_name}' 發生錯誤：{e}"
            )

    # 主程序
    if __name__ == "__main__":
        print(
            f"嘗試刪除 Lambda function：{function_name}"
        )
        delete_lambda_function(function_name)
    ```
