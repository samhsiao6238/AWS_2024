...
_今天課程使用 `90630` 的 `Learner Lab` 操作 EC2，並結合 終端機、VSCode、FileZilla 進行各種連線與檔案傳輸；以下筆記的操作環境為 MacOS 系統，所以沒實作老師講解的 Putty、WinSCP，改用 FileZilla 及 指令 `scp` 進行檔案傳輸，或直接在 VScode 中編輯遠端文件。_
