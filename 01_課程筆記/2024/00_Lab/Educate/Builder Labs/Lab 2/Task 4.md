...
5. 當系統提示選擇新密碼時，輸入自訂密碼如 `Welcome123!`，在這請使用這個密碼避免忘記，也可紀錄在 `MyDoc.txt` 的 `Password` 中，輸入完畢點擊 `Send`；特別說明，Lab 並未指定要輸入這個密碼。
...
6. 返回 Birds 應用程式的瀏覽器頁籤並刷新頁面，現在應只顯示 `teststudent` 的鳥類觀察記錄。整體來說，這個部分是透過更新網站程式碼以動態檢索並過濾資料，成功將 `BirdSightings` 資料表中的數據顯示在應用程式的 `Sightings` 頁面上；學生可查看自己提交的觀察數據，並確保只有自己的數據被顯示。
