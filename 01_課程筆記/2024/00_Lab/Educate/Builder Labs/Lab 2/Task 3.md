...
_返回 Cloud9 IDE 進行操作，這部分僅做檢視，不對文件做任何操作。_
...
2. 先滾動到下方，將代碼中 `<Table-名稱>` 替換為實際資料表名稱；修改後記得儲存變更；特別注意，在官方的腳本中，這個佔位符用 `table_name` 表示，我改為中文純粹是便於從眾多代碼中識別出位置，另外替換後不用保留尖頭括號。
