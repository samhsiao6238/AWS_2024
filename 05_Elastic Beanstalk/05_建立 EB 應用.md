...
6. 顯示訊息，表示平台需要一個服務角色，Elastic Beanstalk 會嘗試創建一個服務角色；也可以使用 --service-role 選項指定自己的角色；按 `ENTER` 繼續，或按 `view` 查看。
...
_創建 Elastic Beanstalk 環境時，可使用 `--service-role` 選項指定服務角色。_
