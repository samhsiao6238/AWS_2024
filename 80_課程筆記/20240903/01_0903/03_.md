...
1. `Amazon SageMaker` 是一個完全托管的機器學習平台，旨在幫助開發者和數據科學家快速構建、訓練和部署機器學習模型，同時提供很多服務和工具來幫助用戶從數據準備、模型訓練到最終部署的全流程機器學習工作流；在這個範例中並未直接使用到 SageMaker 的核心功能，僅用來創建 Jupyter Notebook 實例，也就是 SageMaker Notebook，然後在其中調用 Rekognition API 進行操作；但實際的圖像識別和人臉檢測功能是由 Rekognition 提供的，而不是 SageMaker。

<br>

2. `Amazon Rekognition` 是一個完全托管的計算機視覺服務，使用深度學習模型來執行圖像和視頻的分析和處理，能夠進行各種圖像處理任務，如人臉檢測與識別、物體和場景檢測、文本檢測、情感分析等；在以下範例中，使用 Rekognition 創建人臉集合，然後將人臉圖像添加到集合中，接著搜索目標圖像中的人臉，最後刪除人臉集合。
