...
2. 另外，也可以參考文件 URL 中的字串，這 12 碼數字就是帳號的 ID。
...
4. 第一次登入會提示修改密碼；特別注意，修改之後若為自動進入主控台，就代表所修改的密碼不合規範，一但合乎規範就會自動進入主控台，截至目前（2024/07/21）這裡還有 Bug，不會提示更新失敗。
