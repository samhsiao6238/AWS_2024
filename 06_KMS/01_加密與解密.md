...
4. KMS 服務分為兩種；其一，`AWS managed keys` 是由 AWS 自動管理的密鑰，用於 AWS 服務的預設加密需求，無需用戶手動管理；其二， `Customer managed keys` 是由用戶建立和管理的密鑰，用戶可自定義密鑰的策略、旋轉、啟用、禁用等設置。
...
2. 建立 EBS 卷（volume）時可使用 KMS 加密；EBS（Elastic Block Store）是 AWS 的持久性塊存儲服務，用於為 EC2 提供存儲卷（volume）；而 EC2 是 AWS 的虛擬伺服器服務，用於運行應用和工作負載。
