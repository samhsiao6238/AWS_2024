...
1. 新增兩個腳本 `create_user_and_policy.sh` 及 `create_s3_bucket.sh`。

    ```bash
    touch create_user_and_policy.sh create_s3_bucket.sh
    ```

<br>

2. 編輯腳本 `create_user_and_policy.sh`。
    ...
    # 使用根用戶建立 s3user 並附加政策
    aws iam create-user --user-name s3user --profile default
    aws iam create-access-key --user-name s3user --profile default > temp_access_keys.json

    # 从创建的访问密钥中提取访问密钥 ID 和秘密访问密钥
    AWS_ACCESS_KEY_ID=$(jq -r '.AccessKey.AccessKeyId' temp_access_keys.json)
    AWS_SECRET_ACCESS_KEY=$(jq -r '.AccessKey.SecretAccessKey' temp_access_keys.json)
...
4. 授予腳本執行權限。
    ...
    ```bash
    chmod +x create_user_and_policy.sh create_s3_bucket.sh
    ```
