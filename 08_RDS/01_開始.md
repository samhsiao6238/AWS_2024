...
6. 點擊左側 `Schemas` 會顯示有一個 `sys`。

    ![](images/img_29.png)

<br>

7. 點擊右鍵後在選單點擊 `Refresh all`。

    ![](images/img_30.png)

<br>

8. 在 `Schemas` 會添加所建立的資料庫。

    ![](images/img_31.png)

<br>

9. 在資料庫上點右鍵，然後點擊選單中的 `Table Data Import Wizard` 導入資料。
...
13. 最後一個設定頁面，同樣點擊 `Next`，開始導入資料庫，完成時再點擊一次 `Next` 及 `Finish`。
